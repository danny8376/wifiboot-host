#ifndef _GBAHEADER_H_
#define _GBAHEADER_H_
...
typedef struct
{
	unsigned_int	start_code;			// B instruction
	unsigned char	logo[0xA0-0x04];	// logo data
	unsigned char	title[0xC];			// game title name
	unsigned_int	game_code;			//
	unsigned_short	maker_code;			//
	unsigned char	fixed;				// 0x96
	unsigned char	unit_code;			// 0x00
	unsigned char	device_type;		// 0x00
	unsigned char	unused[7];			//
	unsigned char	game_version;		// 0x00
	unsigned char	complement;			// 800000A0..800000BC
	unsigned_short	checksum;			// 0x0000
} gbaHeader;

typedef struct
{
	unsigned char		padding[4];
	unsigned_int		rom_size;
	unsigned_int		save_type;
	unsigned char		padding2[2];	// 0xFF 0xFF usually?
	unsigned_short		sleep_combo;
	unsigned_int		flash_chip_erase_cycles;
	unsigned_int		flash_sector_erase_cycles;
	unsigned_int		flash_flash_program_cycles;
	unsigned_int		flash_eeprom_write_cycles;
	unsigned_int		lcd_ghosting;
	unsigned char		video_lut[0x300];
	unsigned char		padding3[0xC];
} gbaFooterConfig;

typedef struct
{
	unsigned_int		flag;			// valid 1, dummy 0 (offset need to be 0)
	unsigned_int		offset;			// 0x1000000 => 0x1000000+0?
	unsigned_int		size;			// fixed to 0x324
	unsigned char		padding[4];
} gbaFooterConfigDesc;

typedef struct
{
	gbaFooterConfig		config[1];		// actual config
	gbaFooterConfigDesc	cfg_desc[2];	// almost fixed to 2?
	char				magic[4];		// .CAA
	unsigned_int		fixed;			// 0x1
	unsigned_int		cfg_offset;		// 0x1000330 => 0x1000000+0x330?
	unsigned_int		cfg_count;		// count << 4 => 0x20
} gbaFooter;

unsigned char gba_logo[] = {
...
/*
unsigned char default_video_lut[] = {
	...
	0x2A, 0x2B, 0x2B, 0x2B, 0x2C, 0x2C, 0x2C, 0x2D, 0x2D, 0x2D, 0x2F, 0x2F, 0x2F, 0x30, 0x30, 0x30,
